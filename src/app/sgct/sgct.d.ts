...
/**
 * Determines whether this node should be part of an Nvidia swap group and should use the swap barrier. Please note that this feature only works on Windows and requires Nvidia Quadro cards + G-Sync synchronization cards. For more information on swap groups, see https://www.nvidia.com/content/dam/en-zz/Solutions/design-visualization/quadro-product-literature/Quadro_GSync_install_guide_v4.pdf. The default value is false.
 */
...
/**
 * Determines the pixel resolution of the cube map faces that are individually rendered to create the fisheye rendering. The higher resolution these cube map faces have, the better quality the resulting cylindrical rendering, but this comes at the expense of increased rendering times. The named values are corresponding:
 *     - `low`: 256
 *     - `medium`: 512 (the default)
 *     - `high`: 1024
 *     - `1k`: 1024
 ...
 */
export type CameraFieldOfView = HFovVFov | LeftRightDownUp;
/**
 ...
 * The angle (in degrees) that is covered by the camera between the central point and the left border of the viewport. The `left` and `right` angles added together are the vertical field of view of the viewport.
 */
export type Left4 = number;
/**
 * The angle (in degrees) that is covered by the camera between the central point and the right border of the viewport. The `left` and `right` angles added together are the vertical field of view of the viewport.
 */
export type Right4 = number;
/**
 * The angle (in degrees) that is covered by the camera between the central point and the bottom border of the viewport. The `down` and `up` angles added together are the vertical field of view of the viewport.
 */
 ...
 *     1. SCISS Mesh (`sgc` extension).:A mesh format that was introduced by SCISS for the Uniview software. SCISS created two versions for this file format, one for 2D warping meshes and a second for 3D cubemap lookups. SGCT only supports the first version of the file format, however.
 *     2. Scalable Mesh (`ol` extension): A mesh format created by scalable.
 *     3. Dome Projection (`csv` extension)
 *     4. Paul Bourke Mesh (`data` extension): A file format created by Paul Bourke, his webpage also contains more information abuot the individual steps of the warping.
 *     5. Waveform OBJ (`obj` extension): The well known textual mesh format.
 *     6. SimCAD (`simcad` extension).
 */
...
/**
 * The numerical identifier of this window. By default windows are given numerical value equal to their position in Window's node list, meaning that the first window of a node will have the id `0`, the second window the id `1`, etc. This value can be used to overwrite this behavior. It is not allowed to give the same ID to two different windows.
 */
export type Identifier = number;
/**
 * The name of the window. This is also used as the title of the window if window decorations are enabled. The default name for a window if this value is not specified is "SGCT node: %1 (%2)" with `%1` the address of this node and `%s` either "server" or "client", depending on whether the current node is the server or a client in the cluster.
 */
export type Name2 = string;
/**
 * A comma-separated list of tags that are associated with this window. The tags themselves don't have any meaning inside of SGCT, but can be used by the application code as a filter. One common use-case is to tag one of the windows as a "GUI" window, to restrict input to only that window, for example.
 */
 ...
 *      - `8`: `GL_RGBA8`, `GL_UNSIGNED_BYTE`. This is the default value if nothing else is specified.
...
/**
 * If this value is `false`, this window will not participate in taking screenshots even if commanded so by the user. This can be useful to prevent auxiliary windows to create screenshots in a production environment.
 */
export type TakeScreenshot = boolean;
/**
 ...
 *     1. `none`: No stereo rendering is performed. This is the same as if this entire node was not specified. This is the default value if no other is specified.
 *     2. `active`: Using active stereo using quad buffering. This is only a valid option for systems that support quad buffering.
 *     3. `checkerboard`: Using a checkerboard pattern for stereoscopy in which left and right eyes are rendered on interleaved checkerboard patterns.
 *     4. `checkerboard_inverted`: Using the same pattern as `checkerboard`, but with the left and right eyes inverted.
 *     5. `anaglyph_red_cyan`: Applying color filters to the rendering for the left and right eyes such that red-cyan anaglyph glasses can be used to view the stereo content.
 *     6. `anaglyph_amber_blue`: Applying color filters to the rendering for the left and right eyes such that amber-blue anaglyph glasses can be used to view the stereo content.
 *     7. `anaglyph_wimmer`: Anaglyph method by Peter Wimmer.
 *     8. `vertical_interlaced`: A stereo format in which the left and right eye images are interlaced vertically, meaning that each row of the final image is either left or right, switching each row.
 *     9. `vertical_interlaced_inverted`: The same as `vertical_interlaced`, but with the left and right eye flipped.
 *     10. `dummy`: A dummy stereo mode to test streoscopic rendering without needing extra equipment. In this stereo mode, the left and the right eye images are rendered on top of each other without any other processing. This option is available to verify that stereo rendering is working for a specific application.
 *     11. `side_by_side`: The resolution of the window is split into a left half and a right half, with each eye being rendered into its half. This is a common stereo format for 3D TVs.
 *     12. `side_by_side_inverted`: The same as `side_by_side`, but the left and right images are flipped.
 *     13. `top_bottom`: The same as side_by_side, but instead of separating the window horizontally, the window is split vertically, with the left eye being rendered in the top half of the window and the right image being rendered in the bottom half.
 ...
 */
export type ScalableMesh = string;
/**
 ...
 */
export type Orientation5 = YawPitchRoll | Quaternion;
/**
 * A generic transformation matrix that is applied to this device. This value will overwrite the value specified in `orientation`. All 16 of these values have to be present in this attribute and have to be floating point values and are used in this order to initialize the matrix in a column-major order.
 *
 * @minItems 16
...
/**
 * Sets the size of the window in pixels. This node must have `x` and `y` floating point attributes that determine that size of the window.
 */
 ...
 */
export interface CubemapProjection {
  quality?: Quality;
  spout?: Spout;
  ndi?: NDI;
  channels?: {
...
/**
 * The position of the lower left corner of the projection plane relative to the User's position. The units for these values are arbitrary.
 */
...
/**
 * The position of the upper left corner of the projection plane relative to the User's position. The units for these values are arbitrary.
 */
...
/**
 * The position of the upper right corner of the projection plane relative to the User's position. The units for these values are arbitrary.
 */
...
export interface HFovVFov1 {
  hfov: HorizontalFieldOfView1;
  vfov: VerticalFieldOfView1;
}
export interface LeftRightDownUp1 {
  left: Left4;
  right: Right4;
  down: Down1;
  up: Up1;
...
export interface Spout1 {
  enabled: Enabled;
  name?: Name;
}
export interface NDI1 {
  enabled: Enabled1;
  name?: Name1;
  group?: Group;
}
