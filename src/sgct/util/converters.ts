  ...
  if (!('version' in obj)) {
    ...
    if ('scene' in obj) {
      ...
      if ('orientation' in obj.scene) {
        Object.keys(obj.scene.orientation).forEach((key) => {
        ...
        window.viewports.forEach((viewport: any) => {
          ...
          if ('PlanarProjection' in viewport) {
            ...
            if ('fov' in viewport.projection) {
              Object.keys(viewport.projection.fov).forEach((key) => {
            ...
            if ('orientation' in viewport.projection) {
              Object.keys(viewport.projection.orientation).forEach((key) => {
            ...
            if ('offset' in viewport.projection) {
              Object.keys(viewport.projection.offset).forEach((key) => {
            ...
            if ('crop' in viewport.projection) {
              Object.keys(viewport.projection.crop).forEach((key) => {
            ...
            if ('background' in viewport.projection) {
              Object.keys(viewport.projection.background).forEach((key) => {
            ...
            if ('geometry' in viewport.projection) {
              Object.keys(viewport.projection.geometry).forEach((key) => {
 ...
 */
export async function convertFileVersion(
  content: string,
  extension: string
): Promise<string> {
  let converted = {};
  ...
  if (extension === '.xml') {
    const obj = await convertXmlToJson(content);
